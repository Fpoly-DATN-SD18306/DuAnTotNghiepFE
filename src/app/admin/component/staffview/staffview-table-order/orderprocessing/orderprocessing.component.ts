...
//Promotion
  ...
  addToTemp(product: foodResponse) {
    ...
    if (existingProductInTemp) {
      ...
      let orderDetail: OrderDetailResponse = {
        ...
        quantity: 1,
        price: product.priceFood,
        totalPrice:  Math.round(product.priceFood * ((100 - product.discount) / 100)/1000)*1000,
        noteFood: '',
        discount: product.discount,
      };
    ...
    if (existingProductInList) {
      ...
      existingProductInList.totalPrice =
        Math.round(existingProductInList.price *
        existingProductInList.quantity * ((100 - existingProductInList.discount) / 100)/1000)*1000
  ...
  updateQuantityTemp(item: OrderDetailResponse, newQuantity: number) {
    ...
    item.totalPrice =   Math.round(item.price * item.quantity * ((100 - item.discount) / 100)/1000)*1000;
    this.updateTotal();
